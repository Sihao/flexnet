...
from ..model_interface.model_parameters import (
    rand_u_like,
)
...
class LossGradient(Metric):
    ...
    def __call__(self, model_wrapper: ModelWrapper) -> np.ndarray:
        ...
        gradient = (
            torch.autograd.grad(loss, model_wrapper.named_parameters()).detach().numpy()
        )
...
class LossPerturbations(Metric):
    ...
    def __init__(
        self, loss_fn, inputs: torch.Tensor, target: torch.Tensor, n_directions, alpha
    ):
        super().__init__()
        self.loss_fn = loss_fn
        self.inputs = inputs
    ...
    def __call__(self, model_wrapper: ModelWrapper) -> np.ndarray:
        ...
        start_loss = self.loss_fn(
            model_wrapper.forward(self.inputs), self.target
        ).item()

...
class FullTrajectoryTracker(TrajectoryTracker):
    ...
    def __init__(
        self, model, agent_interface=None, directory="./", experiment_name=None
    ):
        super().__init__()
        self.dir = directory + (
            experiment_name
    ...
    def save_position(self, model):
        np.save(
            ...
            wrap_model(model, self.agent_interface)
            .get_parameter_tensor(deepcopy=True)
            .as_numpy(),
        )
        self.next_idx += 1
...
class ProjectingTrajectoryTracker(TrajectoryTracker):
    ...
    def __init__(self, model, agent_interface=None, n_bases=2):
        ...
        self.A = np.column_stack([np.random.normal(size=n) for _ in range(n_bases)])

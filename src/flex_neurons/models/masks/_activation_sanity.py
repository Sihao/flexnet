...
from .stochastic import StochasticRound, StochasticRoundSigmoid
from src.flex_neurons.models.architectures.simple_conv import SimpleConvNet
...
def compare_configs_on_simple_conv(hyperparams, *configs):
    ...
    if optimizer_name == "adam":
        ...
        optimizers = [
            optim.SGD(
                net.parameters(), lr=learning_rate, weight_decay=1e-5, momentum=0.9
            )
            for net in nets
        ]
    else:
        raise ValueError(f"Optimizer {hyperparams.get('optimizer')} not supported")
...
if __name__ == "__main__":
    compare_configs_on_simple_conv(
        {},
        # STEFunc,
        # STEFuncSigmoid,
        StochasticRound,

...
from ._utils import channel_interpolate, channel_expand_view
...
class Flex2D(nn.Module):
    ...
    def masking(self, logits):
        ...
        elif "StochasticRound" in masking_mechanism:
            mask = getattr(masks, masking_mechanism).apply(logits)
        elif "STE" in masking_mechanism:
            mask = getattr(masks, masking_mechanism).apply(logits)
        else:
            raise ValueError(f"Unknown masking_mechanism: {masking_mechanism}")
...
if __name__ == "__main__":
    ...
    print("Flex2D module loaded. Run tests via: pytest tests/")

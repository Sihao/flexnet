...
# RunLoader lives in the legacy src.analysis tree which is being retired.
# load_experiment_model is the only caller; it needs RunLoader.
# Raise at call time rather than silently breaking import.


class RunLoader:  # pragma: no cover
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            "Legacy import: RunLoader was in src.analysis.run_loader which is retired. "
            "Port this code path to src.flex_neurons.analysis before using load_experiment_model."
        )
